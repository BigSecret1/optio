...
class UserESQuery(ESQuery):
    index_to_search: str = "users_index"
    ...
    def find(self, text):
        ...
        logger.info("Starting a search for word %s", self.query_text)
    ...
    def exact_match(self):
        q = Q('term', **{'first_name.raw': self.query_text})
        matched_results = self._execute_query(q)
        self.__add_to_search_results(matched_results)

    def prefix_match(self):
        q = Q('match_phrase_prefix', **{'first_name.prefix': self.query_text})
        matched_results = self._execute_query(q)
        self.__add_to_search_results(matched_results)

    def substring_match(self):
        q = Q('match', first_name=self.query_text)
        matched_results = self._execute_query(q)
        self.__add_to_search_results(matched_results)
    ...
    def _execute_query(self, query):
        try:
            s = UserESQuery.search.query(query)
            response = s.execute()
            results = []
            for hit in response:
                results.append({
